...
$(document).ready(function(){ // begin document.ready block
  ...
  //       });
        ...
        $.getJSON(url_heat,function(data){ //county boundary data
	            ...
	            //polygons where there's at least one day over 100 in 30 years

	            items_heat_30years = data.features.filter(function(obj) {
				// return the filtered value
				return obj.properties.days_100_today_vs_30yrs === 'zero-to-notzero';
				});

				console.log(items_heat_30years)

	         	var items_heat_30years_style = {
	              "fillColor": "none",
	              "color": "#f95819",
	              "weight": 2,
	              "fillOpacity": 0.9
	            ...
	            heat_30years = L.geoJson(items_heat_30years, {
	                style: items_heat_30years_style,
	                // pane: "boundary",
	                opacity:1,
	                className: "heat_30years"
	            }).addTo(heat_map_1)


	            //polygons where the temp index is over 125 today

	            function heat125todayStyle(feature) {
				    return {
	            ...
	            heat125_today = L.geoJson(items_heat, {
	                style: heat125todayStyle,
	                // pane: "boundary",
	                opacity:1,
	                className: "heat125_today"
	            }).addTo(heat_map_1)


	            //polygons where the temp index in over 125 today

	            function heat12530yearsStyle(feature) {
				    return {
	            ...
	            heat125_30years = L.geoJson(items_heat, {
	                style: heat12530yearsStyle,
	                // pane: "boundary",
	                opacity:1,
	                className: "heat125_30years"
	            }).addTo(heat_map_1)
	...
	$(".scrolly-box-3").waypoint(function(dir){
		...
		if (dir=="down") {
			...
			$(".heat").animate({"opacity":"0"})

			if ($(window).width() >= 900) {
				heat_map_1.flyTo([40.3596928,-99.0598404], 3)
			}
			else if (($(window).width() < 900) & ($(window).width() >= 500)) {
				heat_map_1.flyTo([40.3596928,-99.0598404], 3)
			} 
	...
	$(".scrolly-box-4").waypoint(function(dir){
		...
		if (dir=="down") {
			...
			$(".heat_30years").animate({"opacity":"1"})
			
		}

		else if (dir=="up") {

			$(".heat_30years").animate({"opacity":"0"})
	...
	$(".scrolly-box-5").waypoint(function(dir){
		...
		if (dir=="down") {
			...
			$(".scrolly-map-title").html("<h2>Counties with at least half a day above 125&deg; today</h2>")
			$(".scrolly-map-legend").fadeOut()

			$(".heat").animate({"opacity":"0"})
			$(".heat_30years").animate({"opacity":"0"})
			$(".heat125_today").animate({"opacity":"1"})

		}

		else if (dir=="up") {

			$(".scrolly-map-title").html("<h2>Number of 100&deg; days in 30 years<h2>")
			$(".scrolly-map-legend").fadeIn()

			$(".heat").animate({"opacity":"1"})
			$(".heat_30years").animate({"opacity":"1"})
			$(".heat125_today").animate({"opacity":"0"})
	...
	$(".scrolly-box-6").waypoint(function(dir){
		...
		if (dir=="down") {
			...
			$(".scrolly-map-title").html("<h2>Counties with at least half a day above 125&deg; in 30 years</h2>")
			 
			$(".heat125_30years").animate({"opacity":"1"})
			$(".heat125_today").animate({"opacity":"0"})
			...
			$(".heat125_30years").animate({"opacity":"0"})
			$(".heat125_today").animate({"opacity":"1"})
